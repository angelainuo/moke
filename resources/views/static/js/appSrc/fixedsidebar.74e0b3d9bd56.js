var fixedSidebar = function() {
		...
		var sharehttp =  function (platformName){
			return "http://www.jiathis.com/send/?webid="+platformName+"&url="+encodeURIComponent(location.href)+"&title="+$('#xt_september_newedition_courseabout').find('h3').text()+"&summary="+$('#course_data').find('https://www.xuetangx.com/static/js/appSrc/.course_intro .text').text();
		}
	...
	$(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.statusCode > 0 && res.switch == "1") {
						window.location.href= "http://bad_redirect"
                    }
                    if(res.statusCode > 0 && res.switch == "0") {
                     	window.location.href= "../../../about.htm#contact"/*tpa=https://www.xuetangx.com/about#contact*/
                    }
                },
                error: function(res) {
                	if(res.status == 401) {
		...
		$($("https://www.xuetangx.com/static/js/appSrc/.header_top_bar_text.fl").find("a[data-element=3]")[2]).click(function() {
			fn()
		})
